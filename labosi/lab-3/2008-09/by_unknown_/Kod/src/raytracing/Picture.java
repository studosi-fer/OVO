...
public class Picture extends JFrame {
   ...
   * @param g
   */
  public void paint(Graphics g) {				//pseudokod ray-tracing funkcije iz pripreme
    for (int j=0; j<this.screenResolution; j++) {
      for (int i=0; i<this.screenResolution; i++) {	  
        Point screenPoint=screen.getPoint(i,j);		 //za svaki pixel nabavlja toèku ekrana    
        ray=new Ray(eyePosition, screenPoint);		//napravi se zraka koja ide iz oka promatraèa
        ColorVector colors=scene.traceRay(ray,1);	// pozivanje raytrace funkcije i bojanje toèke
        Color c=new Color(colors.getRed(), colors.getGreen(), colors.getBlue());

        
        g.setColor(c); //tu boju postavljamo

        g.drawRect(i, j, 1, 1); //crtamo jednu toèku ekrana
      }
    }
  }
